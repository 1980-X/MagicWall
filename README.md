...
- 默认以600px高度为基准计算理想宽度（约3017px），如果理想宽度超过屏幕宽度，则以屏幕宽度为准计算高度
...
- **强制使用物理屏幕尺寸**：在生产环境下，窗口尺寸固定为9050px × 1800px（对应实际物理尺寸）
- 在开发环境下，为了方便测试，产品模式使用缩小的尺寸（1920px × 382px），但保持相同的比例
...
- 以 `DEBUG_BASE_HEIGHT` 为基准计算理想宽度（约3017px）
- 如果理想宽度超过屏幕宽度，则以屏幕宽度为准计算高度
- 始终保持与物理屏幕相同的比例（5.028）
